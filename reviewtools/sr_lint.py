...
class SnapReviewLint(SnapReview):
    ...
    def __init__(self, fn, overrides=None):
        ...
        self.redflagged_snap_types = ["base", "kernel", "gadget", "os", "snapd"]

        self.interface_plug_requires_desktop_file = ["unity7", "x11", "unity8"]
    ...
    def check_architectures(self):
        ...
        if not isinstance(self.snap_yaml[key], list):
            t = "error"
            s = "invalid %s entry: %s (not a list)" % (key, self.snap_yaml[key])
        else:
            bad_archs = []
    ...
    def _check_description_summary_title(self, key, maxlen):
        ...
        if not isinstance(self.snap_yaml[key], str):
            ...
            t = "error"
            s = "%s is too long (> %d): '%s'" % (key, maxlen, self.snap_yaml[key])
        self._add_result(t, n, s)
    ...
    def check_name(self):
        ...
        if "name" not in self.snap_yaml:
            ...
            t = "error"
            s = (
                "malformed 'name': '%s' " % self.snap_yaml["name"]
    ...
    def check_type_redflagged(self):
        ...
        if self.snap_yaml["type"] in self.redflagged_snap_types:
            ...
            if (
                snaptype in redflagged_snap_types_overrides
                and pkgname in redflagged_snap_types_overrides[snaptype]
            ):
                s = "OK (override '%s' for 'type: %s')" % (pkgname, snaptype)
            else:
                t = "error"
    ...
    def check_version(self):
        ...
        if "version" not in self.snap_yaml:
            ...
            isinstance(self.snap_yaml["version"], (str, int, float))
            and len(str(self.snap_yaml["version"])) > 32
        ):
            t = "error"
            s = (
                "malformed 'version': '%s' (should be < 32 characters)"
    ...
    def check_valid_hook(self):
        ...
        for fn in hooks:
            ...
            for valid in self.valid_hook_types:
                if hook == valid or (
                    ("-plug-" in hook or "-slot-" in hook) and hook.startswith(valid)
                ):
                    found = True
            if not found:
                unknown.append(hook)
        ...
        if len(unknown) > 0:
            t = "warn"
            s = "unknown hooks in meta/hooks: '%s'" % (",".join(sorted(unknown)))
    ...
    def check_icon(self):
        ...
        if self.snap_yaml["icon"].startswith("/"):
            t = "error"
            s = (
                "icon entry '%s' should not specify absolute path"
                % self.snap_yaml["icon"]
            )
        self._add_result(t, n, s)

        if self.snap_yaml["icon"].startswith(".") or self.snap_yaml[
            "icon"
        ] != os.path.normpath(self.snap_yaml["icon"]):
            t = "error"
            n = self._get_check_name("icon_canonical_path")
            s = "icon entry '%s' does not use canonical path" % self.snap_yaml["icon"]
            self._add_result(t, n, s)
            # return
    ...
    def check_unknown_entries(self):
        ...
        if len(unknown) > 0:
            t = "warn"
            s = "unknown entries in snap.yaml: '%s'" % (",".join(sorted(unknown)))
        self._add_result(t, n, s)
    ...
    def _verify_apps_and_hooks(self, hook=False):
        ...
        if not isinstance(self.snap_yaml[key], dict):
            t = "error"
            s = "invalid %s entry: %s (not a dict)" % (key, self.snap_yaml[key])
            self._add_result(t, n, s)
            return
        ...
        for val in self.snap_yaml[key]:
            ...
            if not isinstance(self.snap_yaml[key][val], dict):
                if key_type == "hook" and self.snap_yaml[key][val] is None:
                    ...
                    t = "error"
                    s = "invalid entry: %s (not a dict)" % (self.snap_yaml[key][val])
                self._add_result(t, n, s)
                continue
            elif key_type == "app" and len(self.snap_yaml[key][val].keys()) < 1:
                t = "error"
    ...
    def _verify_value_is_file(self, app, key, use_shell_quoting=False):
        ...
        if not isinstance(self.snap_yaml["apps"][app][key], str):
            ...
            if (
                "base" in self.snap_yaml
                and self.snap_yaml["base"] == "nix-base"
                and val.startswith("/")
            ):
                val = val.lstrip("/")
            ...
            if fn not in self.pkg_files:
                t = "error"
                s = "%s does not exist" % (self.snap_yaml["apps"][app][key])
        self._add_result(t, n, s)
    ...
    def _verify_apps_hooks_command_chain(self, key, app, hook=False):
        ...
        t = "info"
        n = self._get_check_name("%s_%s" % (topkey_type, key), app=app)
        s = "OK"
        if not isinstance(self.snap_yaml[topkey][app][key], list):
            t = "error"
            s = "%s '%s' (not a list)" % (key, self.snap_yaml[topkey][app][key])
            self._add_result(t, n, s)
            return
        elif len(self.snap_yaml[topkey][app][key]) < 1:
            t = "error"
            s = "%s is empty" % key
            self._add_result(t, n, s)
            return
        ...
        nonexistent = []
        for cmd in self.snap_yaml[topkey][app][key]:
    ...
    def check_apps_command_chain(self):
        ...
        for app in self.snap_yaml["apps"]:
            ...
            if key not in self.snap_yaml["apps"][app]:
                # We check for required elsewhere
                continue
            self._verify_apps_hooks_command_chain(key, app)
    ...
    def check_hooks_command_chain(self):
        ...
        for hook in self.snap_yaml["hooks"]:
            ...
            if key not in self.snap_yaml["hooks"][hook]:
                # We check for required elsewhere
                continue
            self._verify_apps_hooks_command_chain(key, hook, hook=True)
    ...
    def _verify_timeout(self, app, key):
        ...
        n = self._get_check_name("%s" % key, app=app)
        s = "OK"
        if not isinstance(self.snap_yaml["apps"][app][key], int) and not isinstance(
            self.snap_yaml["apps"][app][key], str
        ):
            t = "error"
            s = "'%s' is not a string or integer" % key
        elif not re.search(
            r"[0-9]+(%s)?$" % suffix, str(self.snap_yaml["apps"][app][key])
        ):
            t = "error"
        ...
        s = "OK"
        st = int(re.sub(r"%s" % suffix, "", str(self.snap_yaml["apps"][app][key])))
        if st < 0:
            t = "error"
    ...
    def check_apps_daemon(self):
        ...
        valid = ["simple", "forking", "oneshot", "notify", "dbus"]
    ...
    def check_apps_invalid_combinations(self):
        ...
        for app in self.snap_yaml["apps"]:
            if (
                "daemon" not in self.snap_yaml["apps"][app]
                or self.snap_yaml["apps"][app]["daemon"] != "dbus"
            ):
            ...
            needs_slot = False
            if "slots" not in self.snap_yaml or not self._uses_interface(
                "slots", "dbus"
            ):
                needs_slot = True
            else:
                ...
                for ref in self.snap_yaml["slots"]:
                    ...
                    if (
                        ...
                        and (
                            ("interface" not in spec and ref == "dbus")
                            or ("interface" in spec and spec["interface"] == "dbus")
                        )
                        and "bus" in spec
                    ):
                        if (
                            "slots" in self.snap_yaml["apps"][app]
                            and ref in self.snap_yaml["apps"][app]["slots"]
                            and spec["bus"] == "system"
                            and "name" in spec
                            and spec["name"] == needle
                        ):
            ...
            if needs_slot:
                t = "error"
                s = (
                    "must use a dbus slot that matches 'bus: system' and "
                    + "'name: %s'" % needle
                )
            self._add_result(t, n, s)
        ...
        for app in self.snap_yaml["apps"]:
            show_report = False
            if "slots" not in self.snap_yaml or len(self.snap_yaml["slots"]) == 0:
                continue
            has_session = False
            has_system = False
            for ref in self.snap_yaml["slots"]:
            ...
            if has_session and has_system:
                t = "error"
                s = (
                    "Cannot specify both 'bus: session' and 'bus: system' "
        ...
        for app in self.snap_yaml["apps"]:
            ...
            for ref in self.snap_yaml["slots"]:
                ...
                if (
                    ...
                    and "activatable" in spec
                    and spec["activatable"]
                ):
                    if (
                        "slots" in self.snap_yaml["apps"][app]
                        and ref in self.snap_yaml["apps"][app]["slots"]
            ...
            s = "OK"
            if has_activatable and self.snap_yaml["apps"][app]["daemon"] != "dbus":
                t = "warn"
    ...
    def check_apps_ports(self):
        ...
        for app in self.snap_yaml["apps"]:
            ...
            if not isinstance(self.snap_yaml["apps"][app]["ports"], dict):
                t = "error"
                s = "ports '%s' (not a dict)" % (self.snap_yaml["apps"][app]["ports"])
            ...
            if len(unknown) > 0:
                t = "error"
                n = self._get_check_name("ports_unknown_key", extra=key, app=app)
            ...
            for key in valid_keys:
                ...
                for tagname in self.snap_yaml["apps"][app]["ports"][key]:
                    ...
                    if len(entry.keys()) < 1 or (
                        "negotiable" not in entry and "port" not in entry
                    ):
                        t = "error"
                        n = self._get_check_name("ports", extra=key, app=app)
                        s = "Could not find 'port' or 'negotiable' in '%s'" % tagname
                        self._add_result(t, n, s)
                        continue
                    ...
                    if len(unknown) > 0:
                        ...
                        n = self._get_check_name(
                            "ports_unknown_subkey", extra=key, app=app
                        )
                        s = "Unknown '%s' for %s" % (",".join(unknown), tagname)
                        self._add_result(t, n, s)

                    # port
                    subkey = "port"
                    t = "info"
                    n = self._get_check_name("ports_%s_format" % tagname, extra=subkey)
                    s = "OK"
                    if subkey not in entry:
                        ...
                        if (
                            not port_pat.search(entry[subkey])
                            or int(tmp[0]) < 1
                            or int(tmp[0]) > 65535
                        ):
                            t = "error"
                            s = (
                                "'%s' should be of form " % entry[subkey]
    ...
    def _verify_interfaces(self, iface_type):
        for iface in self.snap_yaml[iface_type]:
            ...
            for attrib in spec:
                ...
                t = "info"
                n = self._get_check_name(
                    "%s_attributes" % iface_type, app=iface, extra=attrib
                )
                s = "OK"
            ...
            for req_iface in self.interfaces_required:
                if (
                    req_iface != interface
                    or iface_type not in self.interfaces_required[interface]
                ):
                ...
                t = "info"
                n = self._get_check_name(
                    "%s_required_attributes" % iface_type, app=iface
                )
                if not has_required:
    ...
    def _verify_app_and_hook_interfaces(self, val, key, hook=False):
        ...
        for ref in self.snap_yaml[topkey][val][key]:
            t = "info"
            n = self._get_check_name(
                "%s_%s_plug_reference" % (topkey_type, key), app=val, extra=ref
            )
            s = "OK"
            if not isinstance(ref, str):
                t = "error"
                s = "invalid %s interface name reference: '%s' (not a str)" % (key, ref)
            elif ref not in self.interfaces and (
                key not in self.snap_yaml or ref not in self.snap_yaml[key]
            ):
                t = "error"
    ...
    def check_hooks_plugs(self):
        ...
        for hook in self.snap_yaml["hooks"]:
            key = "plugs"
            if (
                self.snap_yaml["hooks"][hook] is None
    ...
    def check_external_symlinks(self):
        ...
        if "type" in self.snap_yaml and (
            ...
            or self.snap_yaml["type"] == "os"
            or self.snap_yaml["type"] == "kernel"
        ):
            return
        ...
        s = "OK"
        links = find_external_symlinks(
            self._get_unpack_dir(),
            self.pkg_files,
            self.snap_yaml["name"],
            prefix_ok=prefix_ok,
        )
    ...
    def check_architecture_all(self):
        ...
        if (
            "architectures" in self.snap_yaml
            and "all" not in self.snap_yaml["architectures"]
        ):
            return
    ...
    def check_architecture_specified_needed(self):
        ...
        for arch in self.snap_yaml["architectures"]:
            t = "info"
            n = self._get_check_name("architecture_specified_needed", extra=arch)
            s = "OK"
            if len(self.pkg_bin_files) == 0:
                # This should be a warning but it causes friction for uploads
                t = "info"
                s = "Could not find compiled binaries for architecture '%s'" % arch
            self._add_result(t, n, s)
    ...
    def check_iffy(self):
        ...
        if len(found) > 0:
            t = "warn"
            s = "found potentially sensitive files in package: %s" % ", ".join(found)
        self._add_result(t, n, s)
            ...
            for k in allowed:
                ...
                for i in self.snap_yaml["epoch"][k]:
                    if not isinstance(i, int):
                        t = "error"
                        s = (
                            "malformed full syntax for epoch: '%s' " % k
    ...
    def check_confinement(self):
        ...
        if not isinstance(self.snap_yaml["confinement"], str):
            ...
            t = "info"
            s = (
                "'confinement' should not be used with 'type: %s'"
                % self.snap_yaml["type"]
            )
        self._add_result(t, n, s)
        ...
        if self.snap_yaml["confinement"] == "classic":
            ...
            if (
                ...
                and "snap_allow_classic" in self.overrides
                and self.overrides["snap_allow_classic"]
            ):
                s = "OK (confinement '%s' allowed)" % self.snap_yaml["confinement"]
            else:
                t = "error"
                s = (
                    "(NEEDS REVIEW) confinement '%s' not allowed. If your snap needs classic confinement to function, please make a request for this snap to use classic by creating a new topic in the forum using the 'store-requests' category and detail the technical reasons why classic is required."
                    % self.snap_yaml["confinement"]
                )
                manual_review = True
            ...
            if "plugs" in self.snap_yaml or "slots" in self.snap_yaml:
                ...
                for app in self.snap_yaml["apps"]:
                    if (
                        "plugs" in self.snap_yaml["apps"][app]
                        or "slots" in self.snap_yaml["apps"][app]
                    ):
                        if self.snap_yaml["name"] not in classic_interfaces_exception:
                            found = True
                            break
            if found:
                t = "error"
                s = (
                    "confinement '%s' not allowed with plugs/slots"
                    % self.snap_yaml["confinement"]
                )
    ...
    def check_grade(self):
        ...
        if not isinstance(self.snap_yaml["grade"], str):
            t = "error"
            s = "malformed 'grade': %s (not a string)" % (self.snap_yaml["grade"])
    ...
    def _verify_env(self, env, app=None):
        ...
        for key in env:
            t = "info"
            n = self._get_check_name("environment_key_valid", app=app, extra=key)
            s = "OK"
            link = None
            invalid_chars = []
            for c in invalid:
            ...
            t = "info"
            n = self._get_check_name("environment_value_valid", app=app, extra=key)
            s = "OK"
            if (
                not isinstance(env[key], str)
                and not isinstance(env[key], int)
                and not isinstance(env[key], float)
            ):
                t = "error"
    ...
    def check_apps_environment(self):
        ...
        for app in self.snap_yaml["apps"]:
            ...
            self._verify_env(self.snap_yaml["apps"][app]["environment"], app=app)
    ...
    def check_hooks_environment(self):
        ...
        for app in self.snap_yaml["hooks"]:
            ...
            self._verify_env(self.snap_yaml["hooks"][app]["environment"], app=app)
    ...
    def check_apps_aliases(self):
        ...
        for app in self.snap_yaml["apps"]:
            ...
            if not isinstance(aliases, list):
                ...
                s = (
                    "DEPRECATED: support for using 'aliases' in the yaml "
            ...
            for alias in aliases:
                t = "info"
                n = self._get_check_name("alias_valid", app=app, extra=alias)
                if not pat.search(alias):
                    t = "error"
                    s = (
                        "malformed alias '%s' " % alias
    ...
    def _uses_interface(self, iface_type, iface):
        ...
        if "apps" in self.snap_yaml:
            for app in self.snap_yaml["apps"]:
                if (
                    iface_type in self.snap_yaml["apps"][app]
                    and iface in self.snap_yaml["apps"][app][iface_type]
                ):
                    return True
    ...
    def _verify_icon_path(self, fn):
        ...
        if "/" in fn:
            ...
            if fn.startswith("${SNAP}/"):
                return True, os.path.join(self._get_unpack_dir(), fn.split("/", 1)[1])
            ...
            if fn.startswith("/snap/%s/current/" % self.snap_yaml["name"]):
                return True, os.path.join(self._get_unpack_dir(), fn.split("/", 4)[4])
    ...
    def _verify_desktop_file(self, fn):
        ...
        found_exec = False
        t = "info"
        n = self._get_check_name("desktop_file", app="exec", extra=os.path.basename(fn))
        s = "OK"
        for line in lines:
            line = line.rstrip()
        ...
        for line in lines:
            ...
            t = "info"
            n = self._get_check_name(
                "desktop_file", app=key.lower(), extra=os.path.basename(fn)
            )
            s = "OK"
            if value not in ["true", "false"]:
                t = "warn"
                s = "invalid value for '%s' (should be 'true' or " % key + "'false')"
            self._add_result(t, n, s)

        # Type=Application
        found_type = False
        t = "info"
        n = self._get_check_name("desktop_file", app="type", extra=os.path.basename(fn))
        s = "OK"
        count = 0
        for line in lines:
            line = line.rstrip()
        ...
        for line in lines:
            ...
            t = "info"
            n = self._get_check_name(
                "desktop_file_icon", app=os.path.basename(fn), extra=icon_fn
            )
            s = "OK"
            ...
            if not valid:
                ...
                if icon_fn.startswith("$SNAP/"):
                    s = (
                        "invalid icon path '%s'. Please adjust " % icon_fn
                        + "to use '${SNAP}' instead of '$SNAP'."
                    )
                else:
                    s = (
                        "invalid icon path '%s'. Should either " % icon_fn
                ...
                if not os.path.exists(real_fn):
                    ...
                    t = "error"
                    s = "invalid icon size > %dM" % (self.max_icon_size / 1024 / 1024)
            self._add_result(t, n, s)
    ...
    def check_meta_gui_desktop(self):
        ...
        if self._uses_interface("plugs", "content"):
            if "plugs" in self.snap_yaml:
                for ref in self.snap_yaml["plugs"]:
                    ...
                    if (
                        isinstance(spec, dict)
                        and "interface" in spec
                        and spec["interface"] == "content"
                        and "default-provider" in spec
                        and spec["default-provider"] == "mir-kiosk"
                    ):
        ...
        if len(desktop_interfaces_specified) > 0 and not has_desktop_files:
            if self.snap_yaml["name"] in desktop_file_exception:
                t = "info"
                s = "OK (overidden)"
            elif self._uses_interface("plugs", "x11") and default_provider_is_mir:
                t = "info"
    ...
    def check_base(self):
        ...
        if not isinstance(self.snap_yaml["base"], str):
            t = "error"
            s = "malformed 'base': %s (not a string)" % (self.snap_yaml["base"])
        elif self.snap_yaml["type"] not in use_with:
            t = "error"
            s = "'base' should not be used with 'type: %s'" % self.snap_yaml["type"]
        self._add_result(t, n, s)

        if (
            isinstance(self.snap_yaml["base"], str)
            and self.snap_yaml["base"] in lint_redflagged_base_dep_override
        ):
            t = "info"
            ...
            if (
                self.snap_yaml["name"]
                not in lint_redflagged_base_dep_override[self.snap_yaml["base"]]
            ):
                t = "error"
                s = (
                    "(NEEDS REVIEW) not allowed to specify 'base: %s'"
                    % self.snap_yaml["base"]
                )
                manual_review = True
            self._add_result(t, n, s, manual_review=manual_review)
            ...
            if "apps" in self.snap_yaml:
                for app in self.snap_yaml["apps"]:
                    for j in ["plugs", "slots"]:
                        t = "info"
                        n = self._get_check_name("base_interfaces", app=app, extra=j)
                        s = "OK"
    ...
    def check_license(self):
        ...
        if not isinstance(self.snap_yaml["license"], str):
            t = "error"
            s = "malformed 'license': %s (not a string)" % (self.snap_yaml["license"])
        ...
        for app in self.snap_yaml["apps"]:
            ...
            for sockname in self.snap_yaml["apps"][app][key]:
                ...
                if not self._verify_pkgname(sockname):
                    t = "error"
                    s = (
                        "malformed socket name: '%s' " % sockname
                        + "(may be only lower case, digits and hyphens)"
                    )
                    valid = False
                self._add_result(t, n, s)
                ...
                t = "info"
                n = self._get_check_name(
                    "%s_listen_stream" % key, app=app, extra=sockname
                )
                s = "OK"
                ...
                if port is not None:
                    ...
                    t = "error"
                    s = (
                        "malformed listen-stream '%s' should use one of PORT, ADDR:PORT, @abstract-socket, or PATH"
                        % st
                    )
                    ...
                    if st != os.path.normpath(st):
                        ...
                        t = "error"
                        s = (
                            "listen-stream '%s' filesystem paths does not start with $SNAP_DATA/... or $SNAP_COMMON/..."
                            % st
                        )
                elif st.startswith("@"):  # abstract
                    if not st.startswith("@snap.%s." % self.snap_yaml["name"]):
                        t = "error"
                        s = "abstract socket '%s' does not start with 'snap.%s.'" % (
                            st,
                            self.snap_yaml["name"],
                        )
                else:  # network
                    if not re.search(r"^(127\.0\.0\.1|\[::\]|\[::1\]):[0-9]+", st):
                        t = "error"
                        s = (
                            "network socket '%s' should use one of: 127.0.0.1:PORT, [::]:PORT, [::1]:PORT"
                            % st
                        )
                self._add_result(t, n, s)
                ...
                t = "info"
                n = self._get_check_name(
                    "%s_socket-mode" % key, app=app, extra=sockname
                )
                s = "OK"
                ...
                if mode < 0o1 or mode > 0o777:
                    t = "error"
                    s = "socket-mode '%s' must be within 1-777 octal" % format(
                        mode, "o"
                    )
                elif not st_isfile:
                    t = "error"
        ...
        for slot in self.snap_yaml["slots"]:
            ...
            for key in iface["source"].keys():
                ...
                if key not in ["read", "write"]:
                    ...
                    t = "error"
                    n = self._get_check_name("%s_source_key_valid" % iname, app=key)
                    ...
                    for i in iface["source"][key]:
                        if not isinstance(i, str):
                            t = "error"
                            n = self._get_check_name(
                                "%s_source_item_valid" % iname, app=key
                            )
            ...
            if "read" in iface["source"] and "write" in iface["source"]:
                ...
                if len(both) > 0:
                    t = "error"
                    s = "paths found in both read and write: %s" % ", ".join(both)
                    self._add_result(t, n, s)
    ...
    def check_layout(self):
        ...
        for target in self.snap_yaml[key]:
            ...
            if not isinstance(self.snap_yaml[key][target], dict):
                t = "error"
                s = "invalid entry: %s (not a dict)" % (self.snap_yaml[key][target])
                self._add_result(t, n, s)
                continue
                ...
                t = "error"
                s = (
                    "invalid mount target: '%s'" % target
                    + " (should be "
                    + "a legal path (ie, absolute path or one that starts "
                    + "with: %s)" % ", ".join(snap_vars)
                    + ". If absolute, "
            ...
            known = ["bind", "bind-file", "symlink", "type", "user", "group", "mode"]
            for ltype in self.snap_yaml[key][target]:
                t = "info"
                n = self._get_check_name("%s" % key, app=target, extra=ltype)
                s = "OK"
                ...
                if ltype in ["bind", "bind-file", "symlink"]:
                    ...
                    t = "info"
                    n = self._get_check_name(
                        "%s_source" % key, app=target, extra=source
                    )
                    s = "OK"
                    ...
                    if not isinstance(rdata, int) and not isinstance(rdata, str):
                        t = "error"
                    ...
                    if isinstance(rdata, str):
                        try:
                            ...
                            t = "error"
                            s = (
                                "mode '%s' should be an integer " % rdata
                                + "within 1-777 octal"
                            )
                            self._add_result(t, n, s)
                            continue
                    ...
                    if mode < 0o1 or mode > 0o777:
                        t = "error"
                        s = "mode '%s' must be within 1-777 octal" % format(mode, "o")
                elif ltype == "type":
                    ltype = "type"
                    rdata = self.snap_yaml[key][target][ltype]
    ...
    def check_apps_refresh_mode(self):
        ...
        for app in self.snap_yaml["apps"]:
            ...
            if (
                refresh_mode not in self.valid_refresh_modes
                and refresh_mode not in unknown
            ):
        ...
        if len(unknown) > 0:
            t = "warn"
            s = "unknown refresh-mode: '%s'" % (",".join(sorted(unknown)))
        self._add_result(t, n, s)
    ...
    def check_apps_stop_mode(self):
        ...
        for app in self.snap_yaml["apps"]:
            ...
            if stop_mode not in self.valid_stop_modes and stop_mode not in unknown:
        ...
        if len(unknown) > 0:
            t = "warn"
            s = "unknown stop-mode: '%s'" % (",".join(sorted(unknown)))
        self._add_result(t, n, s)
    ...
    def check_apps_timer(self):
        ...
        for app in self.snap_yaml["apps"]:
            ...
            n = self._get_check_name("%s" % key, app=app)
            s = "OK"
            if not isinstance(self.snap_yaml["apps"][app][key], str) and not isinstance(
                self.snap_yaml["apps"][app][key], int
            ):
                t = "error"
        ...
        for key in ["after", "before"]:
            for app in self.snap_yaml["apps"]:
                ...
                if not isinstance(self.snap_yaml["apps"][app][key], list):
                    t = "error"
                    s = "%s '%s' (not a list)" % (key, self.snap_yaml["apps"][app][key])
                    self._add_result(t, n, s)
                    continue

                if len(self.snap_yaml["apps"][app][key]) == 0:
                    t = "error"
                    s = "%s '%s' (empty)" % (key, self.snap_yaml["apps"][app][key])
                    self._add_result(t, n, s)
                    continue
                ...
                for key_app in self.snap_yaml["apps"][app][key]:
                    if not isinstance(key_app, str):
                        t = "error"
                        n = self._get_check_name("%s" % key, app=app, extra=key_app)
                        s = "%s '%s' (not a str)" % (
                            key,
                            self.snap_yaml["apps"][app][key],
                        )
                    ...
                    t = "info"
                    n = self._get_check_name("%s_exists" % key, app=app, extra=key_app)
                    s = "OK"
    ...
    def check_apps_autostart(self):
        ...
        for app in self.snap_yaml["apps"]:
            ...
            if not isinstance(dname, str):
                ...
                t = "error"
                s = (
                    "malformed desktop file name ('%s'). Should consist of only: [a-zA-Z0-9._-]"
                    % (dname)
                )
            self._add_result(t, n, s)
        ...
        for plug in self.snap_yaml["plugs"]:
            ...
            for attrib in ["read", "write"]:
                ...
                if not isinstance(iface[attrib], list):
                    t = "error"
                    n = self._get_check_name("%s_attrib_valid" % iname, app=attrib)
                ...
                for entry in iface[attrib]:
                    if not isinstance(entry, str):
                        t = "error"
                        n = self._get_check_name("%s_valid" % iname, app=attrib)
                        s = "invalid entry: %s (not a str)" % entry
                        self._add_result(t, n, s)
                        continue

                    t = "info"
                    n = self._get_check_name("%s_path" % iname, app=attrib)
                    s = "OK"
                    if (
                        iname == "personal-files" and not entry.startswith("$HOME/")
                    ) or (iname == "system-files" and not entry.startswith("/")):
                        t = "error"
                        suggested = "/"
                        if iname == "personal-files":
                            suggested = "$HOME"
                        s = (
                            "invalid path for entry: %s " % entry
    ...
    def check_valid_icon_sets(self):
        ...
        icons = glob.glob(
            "%s/meta/gui/icons/**" % self._get_unpack_dir(), recursive=True
        )
        bad_names = []
        unsnap_names = []
        toobig_names = []
        ...
        if len(bad_names) > 0:
            ...
            s = "icon files should not use shell metacharacters: " + ", ".join(
                sorted(bad_names)
            )
            self._add_result(t, n, s)
        ...
        if len(wrongext_names) > 0:
            ...
            s = (
                "invalid icon extension for icon set (should end with "
    ...
    def check_audio_record_without_audio_playback(self):
        ...
        if "apps" not in self.snap_yaml or not self._uses_interface(
            "plugs", "audio-record"
        ):
            return
